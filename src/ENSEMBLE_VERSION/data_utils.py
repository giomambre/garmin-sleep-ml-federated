...
def extract_time_series_features(df):
    ...
    for col in ts_cols:
        ...
        # Use ast.literal_eval safely; handle potential NaNs or non-strings
        def parse_ts(x):
            try:
                if isinstance(x, str):
        ...
        # Calculate stats
        # We use a lambda that returns a Series to expand into multiple columns
        def calc_stats(lst):
            if not lst or len(lst) == 0:
                return pd.Series([np.nan]*5)
            
            # Convert to float array, turn None/non-numeric to NaN
            try:
